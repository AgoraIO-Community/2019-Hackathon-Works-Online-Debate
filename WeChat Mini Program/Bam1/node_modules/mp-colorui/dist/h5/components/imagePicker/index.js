...
import Taro, { chooseImage as _chooseImage, previewImage as _previewImage } from "@tarojs/taro-h5";
import ClIcon from "../icon/index";
import './index.scss';
export default class ClImagePicker extends Taro.Component {
  render() {
    const props = this.props;

    const chooseImgObj = props.chooseImgObj || {};
    const [imgList, setImgList] = Taro.useState(() => {
    ...
    const ChooseImage = () => {
      _chooseImage({
        ...
        success: res => {
          console.log(res);
          const selectArray = res.tempFilePaths.map(url => ({ url, status: 'none' }));
    ...
    const imgComponet = imgList.map((item, index) => <View className="padding-xs bg-img bg-gray" key={item.url} style={{ borderRadius: '6px', position: 'relative' }} onClick={() => {
      viewImage(item.url);
    }}>
      <Image src={item.url} mode="widthFix" style={{ width: '100%', position: 'absolute', left: '0', top: '0', right: '0', bottom: '0' }} />
      {item.status === 'none' ? <View className="cu-tag bg-red" onClick={e => {
        delImg(index, e);
        e.stopPropagation();
    ...
    return <View className="cu-form-group">
