...
import './index.scss';
import ClAnimation from "../animation/index";
import ClLayout from "../layout/index";
let tempPageX = 0;
let tempPageY = 0;
let pageX = 25;
let pageY = 100;
export default class ClFloatButton extends Taro.Component {
  render() {
    const props = this.props;

    const [show, setShow] = Taro.useState(false);
    const [rotate, setRotate] = Taro.useState(0);
    const [animation, setAnimation] = Taro.useState({});
    const [actionListState, setActionListState] = Taro.useState(props.actionList);
    ...
    const actionListComponent = dealActionList.map((item, index) => <View key={item.cu_float_button_id} style={{ position: show ? 'relative' : 'absolute' }}>
      <ClAnimation type={type()} delay={show ? (len - index - 1) / 10 : 0} duration={0.2}>
        <ClLayout padding="small" paddingDirection={direction === 'vertical' ? 'bottom' : 'right'}>
          <View className={`cu-avatar ${shape} ${dealSize} ${item.bgColor ? utils.model.BG_COLOR_LIST[item.bgColor] : dealBgColor} ${dealShadow}`} onClick={e => {
            e.stopPropagation();
            clickButton();
    ...
    return <View className={`${show ? 'float_button__mask' : ''}`} onClick={() => {
      closeWithShadow && clickButton();
    }}>
      <View className="float_button__fixed" style={{
        top: props.position.top && props.position.top !== 'auto' ? Taro.pxTransform(props.position.top) : 'auto',
        right: props.position.right && props.position.right !== 'auto' ? Taro.pxTransform(props.position.right) : 'auto',
        bottom: props.position.bottom && props.position.bottom !== 'auto' ? Taro.pxTransform(props.position.bottom) : 'auto',
        left: props.position.left && props.position.left !== 'auto' ? Taro.pxTransform(props.position.left) : 'auto'
        ...
        if (!move) return;
        const touchs = e.touches[0];
        let newAnimation = _createAnimation(animation);
        ...
        <View className={`float_button__content ${directionClass}`}>
          {actionListComponent}
          <View className={`cu-avatar ${shape} ${dealSize} ${dealBgColor} ${dealShadow}`} onClick={e => {
            e.stopPropagation();
            clickButton();
            onClick && onClick();
          }}>
            <View className={`${dealIconColor}`} style={{
