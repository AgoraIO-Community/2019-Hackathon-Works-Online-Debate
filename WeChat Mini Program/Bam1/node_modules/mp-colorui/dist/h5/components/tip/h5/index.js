...
export default class ClTip_H5 extends Taro.Component {
  render() {
    const props = this.props;

    const [showTip, setShowTip] = Taro.useState(props.show);
    const [distance, setDistance] = Taro.useState({
      top: 'auto',
      left: 'auto',
      right: 'auto',
      bottom: 'auto',
    ...
    const [id, setIdContent] = Taro.useState(() => `cl-tip-content-${Math.random().toString().replace('.', '')}`);
    const [idMessage, setIdMessage] = Taro.useState(() => `cl-tip-message-${Math.random().toString().replace('.', '')}`);
    ...
    Taro.useEffect(() => {
      ...
      const systemInfo = _getSystemInfoSync();
      const res = {
      ...
      if (props.direction === 'top') {
        if (res.top < bottomDistance && res.top < messageData.height) props.direction = 'bottom';
        ...
        if (res.left < res.right && res.left < screenWidth / 2) {
          ...
          distance.arrowTransform = `translateX(${Math.abs(translateX) + res.width / 2}px) rotate(45deg) translateY(0)`;
        } else {
          const maxRight = screenWidth - res.left;
          if (maxRight < contentWidth) translateX = contentWidth - maxRight;
          distance.transform = `translateX(-${translateX}px) translateY(-${res.height + 10}px)`;
          distance.arrowTransform = `translateX(${Math.abs(translateX) + res.width / 2}px) rotate(45deg) translateY(30%)`;
        }
      } else if (props.direction === 'bottom') {
        console.log(res, messageData);
        if (res.top > bottomDistance && bottomDistance < messageData.height) props.direction = 'bottom';
        ...
        if (res.left < contentWidth + 10 && leftDistance > res.left) props.direction = 'right';
        customDirection = [{
          direction: 'right',
          long: 0
        }];
        ...
        if (leftDistance - 10 < contentWidth && leftDistance < res.left) props.direction = 'left';
        const diffHeight = messageData.height - res.height;
    ...
    return <View id={id} onClick={() => {
      ...
      <View id={idMessage} className={`cl-tip__content  shadow ${showTip ? 'show' : 'hide'}`} style={{
        ...
        <View className={`cl-tip__arrow ${props.direction} ${BG_COLOR_LIST[props.bgColor || 'white']} ${showTip ? 'showArrow' : 'hideArrow'}`} style={{
          left: distance.left,
          right: distance.right,
          top: distance.top,
          bottom: distance.bottom,
        ...
        <View className={`cl-tip__message ${BG_COLOR_LIST[props.bgColor || 'white']}`}>
